const CACHE="skaner-studio-offline-release-17-surface-array-cutters";
...
self.addEventListener("fetch",event=>{
  ...
  event.respondWith(caches.match(req,{ignoreSearch:req.mode==="navigate"}).then(hit=>hit||fetch(req).then(response=>{if(response.ok){const copy=response.clone();caches.open(CACHE).then(cache=>cache.put(req,copy));}return response;}).catch(()=>req.mode==="navigate"?caches.match("./"):Response.error())));
});
