const CACHE="skaner-studio-offline-release-89-sculpt-boolean-surface-support";
...
self.addEventListener("install",event=>event.waitUntil((async()=>{
  ...
  const wasm=[],chunks=[];for(const script of scripts){const source=await (await fetch(script)).text();for(const match of source.matchAll(/assets\/[A-Za-z0-9_.-]+\.wasm/g))wasm.push(`./${match[0]}`);for(const match of source.matchAll(/assets\/[A-Za-z0-9_.-]+\.js/g))chunks.push(`./${match[0]}`);for(const match of source.matchAll(/offline-worker-[A-Za-z0-9_.-]+\.js/g))chunks.push(`./assets/${match[0]}`);}
